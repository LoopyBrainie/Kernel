...
| # | Class | Status | Decision |
|---|-------|--------|----------|
| D171 | 1 | ACTIVE | **`neura_` syscall API 前缀** — 跨语言 syscall API 入口 (用户态 shell 可调用的 FFI 符号) 统一 `neura_` 前缀: 10 个 Phase 0 (`neura_open`/`neura_read`/`neura_write`/`neura_close`/`neura_seek`/`neura_stat`/`neura_yield`/`neura_ping`) + 2 个 Phase 1 typed (`neura_pte_map_6arg`/`neura_ipc_send_6arg`) + 1 个 Rust syscall stub (`neura_open_stub`, asm! 块 + D129 约束) + 1 个函数指针类型 (`neura_pte_map_6arg_fn`, D119 arity 检查用)。命名分层: `neura_*` (跨语言 syscall 入口, 用户态 FFI) vs `basal_*` (C HAL 内部函数) vs `cortix_*` (Rust Shell crate, R58 阻塞 Q69)。同步迁移 `cosmo_node_id` → `basal_node_id` (绑 Basal 组件, 非 syscall API)。Back-link: `14-syscall-api.md` § syscall 编号表 (10 处 + 4 个段) + `09-memory-subsystem.md` § D151 FILE_TABLE 段 (4 处) + `15-phase0-mvp.md` T1.1 verify 段 (1 处) + `02-memory-topology.md` § 132KB placeholder 段 (1 处 `basal_node_id`) + `30-open-questions.md` fence-internal (3 处) + `00-naming-taxonomy.md` § 7 D171 表. forbidden-word: `cosmo_open` / `cosmo_read` / `cosmo_write` / `cosmo_close` / `cosmo_seek` / `cosmo_stat` / `cosmo_yield` / `cosmo_ping` / `cosmo_pte_map_6arg` / `cosmo_ipc_send_6arg` / `cosmo_open_stub` / `cosmo_pte_map_6arg_fn` / `cosmo_node_id` 共 13 条 (R59 后入册). D172 豁免: 30-open-questions.md 围栏外历史叙述保留原样, 围栏内当前有效代码同步更新. spec_lab 计划: `R59-M1-syscall-prefix.{sh,_negative.sh}` (text-grep: 14-syscall-api.md syscall 编号表中所有 `neura_*` 必须出现, 0 命中 `cosmo_open`/`cosmo_read`/etc; 反例 grep `cosmo_open` 在 syscall 编号表段必须 0 命中). |
