...
| 03 | design-decisions.md | §一 (D1–D152 full table, R47 ledger closed) | R48 ACTIVE |
...
| 30 | open-questions.md | (Q1–Q67 closed) | R48 ACTIVE (append-only audit history) |

## CI

- `ci/check-docs.sh` — Documentation gate; **N = `${#FORBIDDEN[@]}` 派生** (实时计算, 禁硬编码). 唯一真相源.
- `ci/check-d-backlinks.sh` — D# back-link gate (R48 P0-5 元规则四)
...
5. Run `bash ci/check-docs.sh` to verify the forbidden-word gate still passes (output `0/N` where N is derived live)